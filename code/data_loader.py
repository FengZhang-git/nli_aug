            ...
            for task in range(self.num_tasks):
                ...
                for k in episode_classes:
                    ...
                    for i, s in support.iterrows():
                        tmp = {"text": s['text'], "label": s['class_name'], "aug_text": s['aug_text']}
                        support_set.append(tmp)
                    episode_labels.append(s['class_name'])
                for k in episode_classes:
                    query = df[(df['class_id'] == k) & (~df['id'].isin(support_k[k]['id']))].sample(self.q)
                    for i, q in query.iterrows():
                        tmp = {"text": q['text'], "label": q['class_name'], "aug_text": q['aug_text']}
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        aug_text = self.df['aug_text'][item]

        return text, label, aug_text
